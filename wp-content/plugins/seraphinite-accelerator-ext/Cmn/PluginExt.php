...
class PluginLic
{
	...
	static function Activate( $key )
	{
		...
		$licRes = LicCli::Action( $srvUrl, '594EDF52DE173808712397', LicCli::Act_Activate, $key, 'wordpress-accelerator', '2.21.1', Wp::GetSiteId(), Wp::GetSiteDisplayName() );
		...
		$licInfo = array( 'hr' => $hr, 'key' => $key, 'features' => $licSrvRes[ 'features' ] );
		$data[ 'data' ] = $licInfo;
	...
	static function Deactivate()
	{
		...
		$licRes = LicCli::Action( $srvUrl, '594EDF52DE173808712397', LicCli::Act_Deactivate, $licInfo[ 'key' ], 'wordpress-accelerator', '2.21.1', Wp::GetSiteId() );
		$licSrvRes = $licRes[ 'response' ];
		if( !$licSrvRes )
			$hr = $licRes[ 'hr' ];
		else
	...
	static function Update()
	{
		...
		$licRes = LicCli::Action( $srvUrl, '594EDF52DE173808712397', LicCli::Act_Check, $licInfo[ 'key' ], 'wordpress-accelerator', '2.21.1', Wp::GetSiteId() );
		$licSrvRes = $licRes[ 'response' ];
		if( !$licSrvRes )
			return( $licRes[ 'hr' ] );
		...
		$licInfo[ 'features' ] = (isset($licSrvRes[ 'features' ])?$licSrvRes[ 'features' ]:null);
		$licInfo[ 'hr' ] = $hr;
	...
	static function GetDataUrl( $item )
	{
		...
		$features = (isset($licInfo[ 'features' ])?$licInfo[ 'features' ]:null);
		if( !is_array( $features ) || !count( $features ) )
			return( NULL );

		return( LicCli::GetDataUrl( $srvUrl, '594EDF52DE173808712397', $licInfo[ 'key' ], 'wordpress-accelerator', '2.21.1', Wp::GetSiteId(), $features[ 0 ], $item ) );
	}
	...
	{
		if( is_string( (isset($data[ 'data' ])?$data[ 'data' ]:null) ) )
...
class PluginExt
{
	...
	static function ActivateDeactivateResult( $res )
	{
		...
		$res = (isset($res[ 'activate' ])?$res[ 'activate' ]:null);
		if( $res === null )
			return( null );
		...
		return( Ui::BannerMsg( (isset($msg[ 'severity' ])?$msg[ 'severity' ]:null), (isset($msg[ 'content' ])?$msg[ 'content' ]:null), Ui::MsgOptDismissible ) );
	}
