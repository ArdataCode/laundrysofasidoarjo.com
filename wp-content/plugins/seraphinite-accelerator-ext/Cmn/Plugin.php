...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '2.21.1' || $lastCheckPackage !== 'Ext' )
		{
			$state = Plugin::StateGet();

			if( $lastCheckVer !== '2.21.1' && !isset( $state[ 'changeVerCheck' ] ) )
			{
		...
		if( !$bForce )
		{
			if( $bFirstTimeOnly && $lastCheckVer == '2.21.1' )
				return( Gen::S_FALSE );
...
class Plugin
{
	...
	static function Init()
	{
		...
		add_filter( 'upgrader_source_selection',
			function( $source, $remote_source )
			{
				...
				if( !(isset($sourcePldInfo[ 'UpdateURI' ])?$sourcePldInfo[ 'UpdateURI' ]:null) )
		...
		if( $isAdminMode )
		{
			...
			add_filter( 'plugin_action_links_' . Plugin::GetCurBaseName(),
				function( $actions, $plugin_file )
				{
					...
					if( version_compare( Gen::GetArrField( $rmtCfg, 'Info.LastVersion', '2.21.1' ), '2.21.1' ) > 0 )
					{
	...
	static function AsyncTaskPushEx( $urlPush, $timeout = Plugin::ASYNCTASK_PUSH_TIMEOUT )
	{
		...
		$asyncMode = Gen::CallFunc( 'seraph_accel\\OnAsyncTasksPushGetMode' );
		if( $asyncMode == 're' )
			return( PluginRe::MakeRequest( $method, $urlPush, array(  ), $timeout ) );
	...
	static private function _UpdateAndGetExtTool( &$mdl, $name, $ver, $osFileSuffix, $force = false )
	{
...

		{
			...
			{
				Gen::LastErrDsc_Set( LocId::Pack( 'FileReadErr_%1$s', 'Common', array( $mdl ) ) );
				$lock -> Release();
				return( Gen::E_FAIL );
			}
	...
	static function GetAboutPluginContent()
	{
		...
		$urlAboutPluginImg = file_exists( __DIR__ . '/../Images/ProductLogo.png' ) ? add_query_arg( array( 'v' => '2.21.1' ), Plugin::FileUri( '../Images/ProductLogo.png', __FILE__ ) ) : null;
		...
		$url3rdPartySoft = file_exists( __DIR__ . '/../third-party-software.html' ) ? add_query_arg( array( 'v' => '2.21.1' ), Plugin::FileUri( '../third-party-software.html', __FILE__ ) ) : null;
	...
	static function GetAboutVendorContent()
	{
		...
		$urlAboutUsLogoImg = file_exists( __DIR__ . '/../Images/VendorLogo.png' ) ? add_query_arg( array( 'v' => '2.21.1' ), Plugin::FileUri( '../Images/VendorLogo.png', __FILE__ ) ) : null;
	...
	static private function _PrevVer_Check()
	{
		...
		if( empty( $warningVersionInfo ) )
		{
			...
			$verTo = self::_PrevVer_GetInt( '2.21.1' );
			if( $verTo < $verFrom )
...
?>
	...
	static private function _TransientUpdatePlugins_Process( &$value, $plgVer, $switch )
	{
		...
		if( $switch )
			$dwnldUrl = Gen::GetFileName( $dwnldUrl, true, true ) . '.2.21.1.' . Gen::GetFileExt( $dwnldUrl );
		else
		{
			...
			add_filter( 'http_request_args',
				function( $parsed_args, $url )
				{
					...
					if( isset( $parsed_args[ 'body' ][ 'plugins' ] ) )
					{
						...
						if( is_array( (isset($to_send[ 'active' ])?$to_send[ 'active' ]:null) ) && !in_array( Plugin::BASENAME, $to_send[ 'active' ] ) )
						{
		...
		if( ( $switch || version_compare( $verLast, '2.21.1' ) > 0 ) && !isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
			...
			else
			{
				...
				$value -> response[ Plugin::GetCurBaseName() ] = $obj;
			}
		...
		if( isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
			...
			if( !$switch && version_compare( $verLast, '2.21.1' ) > 0 )
			...
			$cAdjuster -> onRequest = function( $obj, $url, $p1, $p2, $p3, &$options )
			{
				...
				if( (isset($options[ 'connect_timeout' ])?$options[ 'connect_timeout' ]:null) < 60 )
					$options[ 'connect_timeout' ] = 60;
				if( (isset($options[ 'timeout' ])?$options[ 'timeout' ]:null) < 60 )
					$options[ 'timeout' ] = 60;
			};
	...
	static function CmnStyle( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_enqueue_style( Plugin::CmnScriptId( $id ), add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.css' ), array(), '2.21.1' );
	...
	static function CmnScripts( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_register_script( $scrHndId, add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.js' ), $deps, '2.21.1' );
			if( $id == 'Gen' )
	...
	static function _on_update_plugin_get_info( $value, $plugin_data, $plugin_file, $locales )
	{
		...
		if( $info = (isset($data -> response[ Plugin::GetCurBaseName() ])?$data -> response[ Plugin::GetCurBaseName() ]:null) )
		{
	...
	static function _on_site_transient_update_plugins( $value, $transient )
	{
		...
		$switchTo = Wp::SanitizeId( (isset($_REQUEST[ 'seraph_accel_switchto' ])?$_REQUEST[ 'seraph_accel_switchto' ]:null) );
		if( !empty( $switchTo ) )
		{
			$plgVer = $switchTo;
			$switch = true;
				...
				<script>
					(function()
					{
						...
						function _ProcessAnswerAndDeactivateAndClose( answerId, answerUserData )
						{
							...
							sendDataUrl = sendDataUrl.replace( "{PluginVersion}",				encodeURI( "2.21.1" ) );
...
class PluginRe
{
	...
	static function Exec( $sMethod, $sUrl, $aHdrs, $nWaitForEnd = true, $bBody = true, &$sBody = null )
	{
		...
		$sCmd = Gen::ExecEscArg( $sPhpLauncher ) . ' -c ' . Gen::ExecEscArg( php_ini_loaded_file() ) . ' -d opcache.file_cache=' . Gen::ExecEscArg( $dirOpCache ) . ' -d opcache.enable_cli=1 -d opcache.file_cache_only=1 -d opcache.optimization_level=' . ini_get( 'opcache.optimization_level' ) . ' -d display_errors=' . ini_get( 'display_errors' ) . ' -d output_buffering=' . ini_get( 'output_buffering' ) . ' -d memory_limit=' . ini_get( 'memory_limit' ) . ' -d register_argc_argv=1 ' . Gen::ExecEscArg( dirname( __DIR__ ) . '/re.php' ) . ( is_int( $nWaitForEnd ) ? ( ' --t=' . $nWaitForEnd ) : '' ) . ( $fileOutTmt ? ( ' --tmp=' . Gen::ExecEscArg( $fileOutTmt ) ) : '' ) . ' --root=' . Gen::ExecEscArg( $_SERVER[ 'DOCUMENT_ROOT' ] ) . ' --soft=' . Gen::ExecEscArg( Gen::ExecMaskUrlArg( rawurlencode( (isset($_SERVER[ 'SERVER_SOFTWARE' ])?$_SERVER[ 'SERVER_SOFTWARE' ]:'') ) ) ) . ' --method=' . Gen::ExecEscArg( $sMethod ) . ' --url=' . Gen::ExecEscArg( Gen::ExecMaskUrlArg( $sUrl ) ) . ' --hdrs=' . Gen::ExecEscArg( Gen::ExecMaskUrlArg( rawurlencode( @json_encode( ( array )$aHdrs ) ) ) );
		...
		$hProc = @proc_open( $sCmd, array( 1 => array( 'file', $fileOut, 'w' ), 2 => array( 'file', $fileOut, 'w' ) ), $aPipe );
		if( !$hProc )
		{
			if( $fileOutTmt )
				@unlink( $fileOutTmt );
		...
		if( strlen( $sBody ) )
		{
			...
			if( $iSfxPosEnd !== false )
			{
				...
				if( $iSfxPos !== false )
				{
					...
					$hr = (isset($aSfxRes[ 'hr' ])?$aSfxRes[ 'hr' ]:Gen::E_DATACORRUPTED);
	...
	static function OpCacheReset()
	{
		...
		$dirOpCache = self::_GetOpCacheDir( $sPhpLauncher, Gen::GetTempDirEx() );
	...
	static function _GetPhpLauncherEx( $sExtensionDir, $sPhpOs )
	{
		...
		if( 0 ) {}
		else if( preg_match( '@/php/?\\d+/(lib/php/(?:ext|mod|\\d+))@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
		else if( preg_match( '@/php/?[\\d.]+/(lib(?:64)?/php/(?:ext|mod))@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
		else if( preg_match( '@/php/(lib/php/(?:ext|mod))@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
		else if( preg_match( '@/usr/(lib(?:64)?(?:/php)?/modules)@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
