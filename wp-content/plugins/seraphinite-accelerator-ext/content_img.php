...
function Images_ProcessSrcEx( &$ctxProcess, $imgSrc, $settCache, $settImg )
{
	...
	{
		if( (isset($ctxProcess[ 'debugM' ])?$ctxProcess[ 'debugM' ]:null) )
			LastWarnDscs_Add( LocId::Pack( 'FileReadErr_%1$s', 'Common', array( $file ) ) );
		return( null );
	}
...
function Images_ProcessSrcSizeAlternatives( $imgSzAlternatives, &$ctxProcess, $imgSrc, $settCache, $settImg, $settCdn, $bCrop = false, $isImportant = false )
{
	...
	static $g_aAdaptDimsScale = array( 1.0, 	1.01,	1.01,	1.03,	1.05,	1.0,	1.0	 );

	$szAdaptBgCxMin = Gen::GetArrField( $settImg, array( 'szAdaptBgCxMin' ), 0 );
	foreach( array( $info[ 'cx' ], 			1920,	1366,	992,	768,	480,	360	 ) as $i => $cx )
	{
		if( $infoCache === false )
			break;
...
function Images_Process( &$ctxProcess, $doc, $settCache, $settImg, $settCdn )
{
	...
	foreach( $items as $item )
	{
		...
		if( !$item -> attributes || HtmlNd::FindUpByTag( $item, 'noscript' ) )
			continue;
	...
	{
		if( !HtmlNd::FindUpByTag( $itemSvgImg, 'svg' ) || HtmlNd::FindUpByTag( $item, 'noscript' ) )
			continue;
...
{
	return( array( 'seraph_accel_gci' => $path, 'n' => Gen::GetNonce( $path, NONCE_SALT ) ) );
}
...
function Images_ProcessSrcSizeAlternatives_Cache_Get( $dataPath, $imgStgId )
{
	...
	$lock = new Lock( $fileTpl . 'l', false, true );
	if( !$lock -> Acquire() )
		return( null );
