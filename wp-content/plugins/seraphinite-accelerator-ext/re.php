...
if( isset( $aOpts[ 't' ] ) )
{
	...
	$sCmd = \seraph_accel\Gen::ExecEscArg( PHP_BINARY ) . ' -c ' . \seraph_accel\Gen::ExecEscArg( php_ini_loaded_file() ) . ' -d opcache.file_cache=' . \seraph_accel\Gen::ExecEscArg( ini_get( 'opcache.file_cache' ) ) . ' -d opcache.enable_cli=' . ini_get( 'opcache.enable_cli' ) . ' -d opcache.file_cache_only=' . ini_get( 'opcache.file_cache_only' ) . ' -d opcache.optimization_level=' . ini_get( 'opcache.optimization_level' ) . ' -d display_errors=' . ini_get( 'display_errors' ) . ' -d output_buffering=' . ini_get( 'output_buffering' ) . ' -d memory_limit=' . ini_get( 'memory_limit' ) . ' -d register_argc_argv=' . ini_get( 'register_argc_argv' ) . ' ' . \seraph_accel\Gen::ExecEscArg( __FILE__ ) . ' --root=' . \seraph_accel\Gen::ExecEscArg( (isset($aOpts[ 'root' ])?$aOpts[ 'root' ]:'') ) . ' --soft=' . \seraph_accel\Gen::ExecEscArg( (isset($aOpts[ 'soft' ])?$aOpts[ 'soft' ]:'') ) . ' --method=' . \seraph_accel\Gen::ExecEscArg( (isset($aOpts[ 'method' ])?$aOpts[ 'method' ]:'') ) . ' --url=' . \seraph_accel\Gen::ExecEscArg( (isset($aOpts[ 'url' ])?$aOpts[ 'url' ]:'') ) . ' --hdrs=' . \seraph_accel\Gen::ExecEscArg( (isset($aOpts[ 'hdrs' ])?$aOpts[ 'hdrs' ]:'') );
	...
	$hProc = @proc_open( $sCmd, array( 1 => array( 'file', $sFileOut, 'w' ) ), $aPipe );
	if( !$hProc )
...
{
	echo( 'E_INVALIDARG: url: "' . (isset($aOpts[ 'url' ])?$aOpts[ 'url' ]:'') . '"' );
...

{
	...
	$_SERVER[ 'SERVER_SOFTWARE' ] = ( strlen( $aOpts[ 'soft' ] ) ? rawurldecode( _seraph_accel_re_Rtn::ExecUnMaskUrlArg( $aOpts[ 'soft' ] ) ) : 'Generic' ) . ' (Seraph Accel Request Emulator 2.21.1)';
...
if( !isset( $_SERVER[ 'HTTP_USER_AGENT' ] ) )
	$_SERVER[ 'HTTP_USER_AGENT' ] = 'Seraph Accel Request Emulator 2.21.1';
