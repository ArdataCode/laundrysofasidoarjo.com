...
spl_autoload_register(
	function( $class )
	{
		if( strpos( $class, 'PhpCss' ) === 0 )
			require_once( 'D:/Data/Temp/PhpCss/' . str_replace( '\\', '/', $class ) . '.php' );
	}
);
...
class StyleProcessor
{
		...
		for( $item = null; $item = HtmlNd::GetNextTreeChild( $this -> doc, $item );  )
		{
			...
			if( !$init )
			{
				if( $item -> hasAttribute( 'class' ) )
					$item -> setAttribute( 'class', str_replace( array( '| ', ' |' ), array( '', '' ), $item -> getAttribute( 'class' ) ) );
			...
			if( $item -> hasAttribute( 'class' ) )
			{
				...
				$item -> setAttribute( 'class', '| ' . $sClasses . ' |' );
			}
			else
				$sClasses = null;
	...
	static function ImgSzAlternativesGetStyleBlocks( $ctxProcess, $aSel, $imgSzAlternatives, $important = false, $bLazyBg = false )
	{
		...
		foreach( $imgSzAlternatives -> a as $dim => $imgSzAlternative )
		{
...

			{
				...
				$ruleAdd -> setIsImportant( $important || $imgSzAlternatives -> isImportant );
				$block -> addRule( $ruleAdd );
				unset( $ruleAdd );
			}
	...
	static function AdjustItemAdaptImg( &$ctxProcess, $settImg, $doc, $item, $itemCssClass = null )
	{
		...
		static $g_defSizes = null;
		{
			...
			foreach( array( 'O', 			1920,	1366,	992,	768,	480,	360	 ) as $dim )
			{
				if( $dim !== 'O' )
					$g_defSizes .= '-' . $dim;
				foreach( array( 			1920,	1366,	992,	768,	480,	360	 ) as $dimCrop )
					if( $dim === 'O' || $dimCrop < $dim )
						$g_defSizes .= '-' . $dim . '@' . $dimCrop;
			}
	...
	function _trace( $e )
	{
		$eS = ( $e instanceof Sabberworm\CSS\Parsing\SourceException ) ? $e : null;
