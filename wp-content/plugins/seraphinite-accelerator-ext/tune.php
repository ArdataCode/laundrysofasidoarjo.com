...
function _SettingsWizardPage()
{
	...
	wp_register_script( Plugin::ScriptId( 'Admin' ), add_query_arg( Plugin::GetFileUrlPackageParams(), Plugin::FileUrl( 'Admin.js', __FILE__ ) ), array_merge( array( 'jquery' ), Plugin::CmnScriptId( array( 'Cmn', 'Gen', 'Ui', 'Net' ) ) ), '2.21.1' );
		...
		function( $callbacks_args, $box )
		{
			...
			echo( Ui::TagOpen( 'div', array( 'class' => 'wizPages' ) ) );
			{
				...
				echo( Ui::TagOpen( 'div' ) );
				{
					$fldId = 'cdn';
					$fldIdEx = 'cdnAddr';
					$vDef = $vDefDontChange ? '' : 'no';

					echo( Ui::Tag( 'p', Wp::safe_html_x( 'Info', 'admin.SetupWizard_Cdn', 'seraphinite-accelerator' ) ) );

					echo( Ui::TagOpen( 'p' ) );
					{
						echo( Ui::RadioBox( Wp::safe_html_x( 'NoRad', 'admin.SetupWizard_Cdn', 'seraphinite-accelerator' ), 'seraph_accel/' . $fldId, 'no', $vDef == 'no' ) );
						echo( Ui::Tag( 'span', Wp::safe_html_x( 'NoInfo', 'admin.SetupWizard_Cdn', 'seraphinite-accelerator' ), array( 'class' => 'chkradSubBlock' ) ) );
					}
					echo( Ui::TagClose( 'p' ) );

					echo( Ui::TagOpen( 'p' ) );
					{
						echo( Ui::RadioBox( Wp::safe_html_x( 'UseRad', 'admin.SetupWizard_Cdn', 'seraphinite-accelerator' ), 'seraph_accel/' . $fldId, 'use', $vDef == 'use' ) );
						echo( Ui::Tag( 'span', Wp::safe_html_x( 'UseInfo', 'admin.SetupWizard_Cdn', 'seraphinite-accelerator' ) . '<br><br>' . _SettCdnAddrEditor( 'seraph_accel/' . $fldIdEx ), array( 'class' => 'chkradSubBlock' ) ) );
					}
					echo( Ui::TagClose( 'p' ) );
...
function _OnFinishSettingsWizard( $args )
{
...

	{
		...
		if( $v !== null )
		{
			...
			{
				if( (isset($item[ 'id' ])?$item[ 'id' ]:null) == 'mobile' )
				{
					$settItemDef = $item;
					break;
				}
	...
	{
		$fldId1 = array( 'contPr', 'cdn', 'items' );

		$val = null;
		switch( $args[ 'seraph_accel/cdn' ] )
		{
		case 'use':			$val = @rtrim( @trim( Wp::SanitizeUrl( (isset($args[ 'seraph_accel/cdnAddr' ])?$args[ 'seraph_accel/cdnAddr' ]:null) ) ), '/' ); break;
		case 'no':			$val = ''; break;
		}

		if( $val !== null )
		{
			foreach( Gen::GetArrField( $sett, $fldId1, array() ) as $i => $item )
				Gen::SetArrField( $sett, array_merge( $fldId1, array( $i, 'enable' ) ), false );

			Gen::SetArrField( $sett, array_merge( $fldId1, array( 0 ) ), Gen::GetArrField( $settDef, array_merge( $fldId1, array( 0 ) ), array() ) );
			Gen::SetArrField( $sett, array_merge( $fldId1, array( 0, 'addr' ) ), $val );
		}
	}
...
function _SelfDiag_GetResponseResString( $url, $requestRes, $body = false )
{
	...
	if( (isset($requestRes[ 'url' ])?$requestRes[ 'url' ]:null) )
		$url = $requestRes[ 'url' ];
...
function OnAdminApi_SelfDiag_AsyncRequest( $args )
{
	...
	else
	{
		...
		else
		{
			...
			{
				$res -> hr = Gen::S_FAIL;
				$res -> descr = Wp::safe_html_x( 'TaskExecTimeoutExpired', 'admin.SelfDiag', 'seraphinite-accelerator' );
			}
		}
...
function OnAdminApi_SelfDiag_SetMaxExecTime( $args )
{
	...
	if( !@set_time_limit( $procTmLim + 30 ) )
	{
...
function OnAdminApi_SelfDiag_PageOptimize( $args )
{
	...
	$url = add_query_arg( array( 'seraph_accel_prep' => @rawurlencode( @base64_encode( @json_encode( array( 'nonce' => hash_hmac( 'md5', '' . $tmBegin, NONCE_SALT ), 'selfTest' => $selfTestSuffix, '_tm' => '' . $tmBegin ) ) ) ) ), Wp::GetSiteRootUrl() );

	$asyncMode = OnAsyncTasksPushGetMode();
	if( $asyncMode == 're' )
		$requestRes = PluginRe::MakeRequest( 'GET', $url, array(), 45 );
	else

		$requestRes = Wp::RemoteGet( $url, array( 'local' => $asyncMode == 'loc', 'timeout' => 45, 'sslverify' => false ) );
...
function OnAdminApi_SelfDiag_ExtCache( $args )
{
	...
	else if( wp_remote_retrieve_header( $requestRes, 'x-seraph-accel-test' ) !== $tmStamp )
	{
		if( !$isExtCacheAllowed )
		{
			...
			$types = array();
			if( strpos( wp_remote_retrieve_header( $requestRes, 'server' ), 'nginx' ) === 0 && ( wp_remote_retrieve_header( $requestRes, 'x-cache' ) || wp_remote_retrieve_header( $requestRes, 'x-cache-status' ) ) )
				$types[] = 'Nginx';
			if( wp_remote_retrieve_header( $requestRes, 'x-litespeed-cache' ) )
				$types[] = 'LiteSpeed';
			if( wp_remote_retrieve_header( $requestRes, 'x-fastcgi-cache' ) )
				$types[] = 'FastCGI';
			if( wp_remote_retrieve_header( $requestRes, 'cf-cache-status' ) && wp_remote_retrieve_header( $requestRes, 'cf-ray' ) )
				$types[] = 'CloudFlare';
			if( wp_remote_retrieve_header( $requestRes, 'x-ezoic-cdn' ) )
				$types[] = 'Ezoic CDN';
...
function SelfDiag_DetectStateAnd3rdPartySettConflicts( $cb, $ext = false )
{
	...
	if( $plg && (isset($plg[ 'IsActive' ])?$plg[ 'IsActive' ]:null) )
	{
		...
		if( $isScriptsDelayLoadEnabled && get_option( 'elementor_experiment-e_lazyload' ) == 'active' )
	...
	if( $plg && (isset($plg[ 'IsActive' ])?$plg[ 'IsActive' ]:null) )
	{
		...
		if( $isScriptsDelayLoadEnabled )
		{
			if( _3rdParty_to_boolean( Gen::GetArrField( $plgOpts, 'lazyonbg', 'false' ) ) )
			...
			if( defined( 'RS_REVISION' ) && version_compare( RS_REVISION, '6.5' ) >= 0 && !in_array( Gen::GetArrField( $plgOpts, 'forceLazyLoading', 'smart' ), array( '', 'none' ), true ) )
	...
	if( $plg && (isset($plg[ 'IsActive' ])?$plg[ 'IsActive' ]:null) )
	{
		...
		if( $isScriptsDelayLoadEnabled && in_array( 'lazy-images', $plgOpts ) )
