...
function OnEarlyContentComplete( $buffer, $tmpUpdate = false )
{
	...
	{
		if( !headers_sent() )
			header( 'X-Seraph-Accel-Content: 2.21.1; status=' . $skipStatus . ', procTime=' . $tmProc . 's' );
...
function ContentProcess_IsItemInFragments( $ctxProcess, $item, $strict = false )
{
	if( $ctxProcess[ 'mode' ] != 'fragments' )
		return( true );

	foreach( $ctxProcess[ 'fragments' ] as $itemFragment )
		if( $strict ? ( $itemFragment === $item ) : HtmlNd::DoesContain( $itemFragment, $item ) )
			return( true );
...
function ContentProcess( &$ctxProcess, $sett, $settCache, $settContPr, $buffer, &$skipStatus )
{
		...
		if( (isset($ctxProcess[ 'lazyload' ])?$ctxProcess[ 'lazyload' ]:null) || (isset($ctxProcess[ 'imgAdaptive' ])?$ctxProcess[ 'imgAdaptive' ]:null) )
		{
...

			{
				...
				if( (isset($ctxProcess[ 'imgAdaptive' ])?$ctxProcess[ 'imgAdaptive' ]:null) )
				{
					...
					$cont .= "(function(h,k){function n(a){if(!a)return[];for(var c=[],b=[a.szOrig[0],1920,1366,992,768,480,360],e=0;e<b.length;e++){var d=b[e];if(!(e&&d>=a.szOrig[0])){if((e||!a.nRenderMinRatio)&&a.cx>d)break;e&&c.push(\"\"+d);if(a.nRenderMinRatio||a.cxRenderMin)for(var f=1;f<b.length;f++){var g=b[f];if(!(g>=d)){if(a.nRenderMinRatio&&a.nRenderMinRatio>g/(d/a.szOrig[0]*a.szOrig[1]))break;if(a.cxRenderMin&&a.cxRenderMin>g)break;c.push(\"\"+(e?d:\"O\")+\"@\"+g)}}}}return c}function p(a,c=!1){if(c||!a.classList.contains(\"lzl\")&&\n!a.classList.contains(\"lzl-ing\")){try{var b=JSON.parse(a.getAttribute(\"data-ai-img\"))}catch(v){}if(b){for(var e=n({cx:a.clientWidth,szOrig:b.s}),d=b.o,f=e.length;0<f;f--){var g=b.d[e[f-1]];if(g){d=g;break}}b=\"src\";c&&a.hasAttribute(\"data-lzl-src\")&&(b=\"data-lzl-src\");a.getAttribute(b)!=d&&a.setAttribute(b,d)}}}function q(a){var c=getComputedStyle(a);if(\"fixed\"==c.getPropertyValue(\"background-attachment\"))var b=[window.visualViewport.width,window.visualViewport.height];else{b=[a.clientWidth,a.clientHeight];\nvar e=c.getPropertyValue(\"background-origin\");if(\"content-box\"==e||\"padding-box\"==e)b[0]-=parseInt(c.getPropertyValue(\"border-left-width\"),10)+parseInt(c.getPropertyValue(\"border-right-width\"),10),b[1]-=parseInt(c.getPropertyValue(\"border-top-width\"),10)+parseInt(c.getPropertyValue(\"border-bottom-width\"),10);\"content-box\"==e&&(b[0]-=parseInt(c.getPropertyValue(\"padding-left\"),10)+parseInt(c.getPropertyValue(\"padding-right\"),10),b[1]-=parseInt(c.getPropertyValue(\"padding-top\"),10)+parseInt(c.getPropertyValue(\"padding-bottom\"),\n10))}if(0>=b[0]||0>=b[1])c=void 0;else{var d=c.getPropertyValue(\"--ai-bg-sz\").split(\" \");if(d[1]){d[0]=parseInt(d[0],10);d[1]=parseInt(d[1],10);var f=c.getPropertyValue(\"background-size\");e={szOrig:d};var g=null;\"auto\"==f?(e.cx=d[0],g=\"auto\"):\"contain\"==f?(f=b[0]/b[1],d=d[0]/d[1],e.cx=d>f?b[0]:Math.round(d*b[1])):\"cover\"==f?(f=b[0]/b[1],d=d[0]/d[1],f>d?e.cx=b[0]:(e.cx=Math.round(d*b[1]),g=\"cover\")):(f=f.split(\" \")[0],f.lastIndexOf(\"%\")==f.length-1?e.cx=Math.round(parseInt(f,10)/100*b[0]):f.lastIndexOf(\"px\")==\nf.length-2?e.cx=parseInt(f,10):e.cx=999999999);g&&\"50%\"==c.getPropertyValue(\"background-position-x\")&&(\"auto\"==g?e.cxRenderMin=b[0]:\"cover\"==g&&(e.nRenderMinRatio=b[0]/b[1]));c=e}else c=void 0}c=n(c);c.length||c.push(\"O\");c=\"-\"+c.join(\"-\")+\"-\";a.getAttribute(\"data-ai-bg\")!=c&&a.setAttribute(\"data-ai-bg\",c)}function l(){m||(m=setTimeout(function(){m=void 0;r()},250))}function r(){var a;for(a=0;a<t.length;a++)p(t[a]);for(a=0;a<u.length;a++)q(u[a])}k.lzl_lazysizesConfig.beforeCheckElem=function(a){a.classList.contains(\"ai-img\")&&\np(a,!0);a.classList.contains(\"ai-bg\")&&q(a)};var u=h.getElementsByClassName(\"ai-bg\"),t=h.getElementsByClassName(\"ai-img\"),m;h.addEventListener(\"DOMContentLoaded\",r,!1);k.addEventListener(\"hashchange\",l,!0);k.addEventListener(\"resize\",l,!1);k.MutationObserver?(new k.MutationObserver(l)).observe(h.documentElement,{childList:!0,subtree:!0,attributes:!0}):(h.documentElement.addEventListener(\"DOMNodeInserted\",l,!0),h.documentElement.addEventListener(\"DOMAttrModified\",l,!0))})(document,window);";
				}

				if( $bLazyCont )
				{

					$cont .= str_replace( array( '_URL_GET_CONTPARTS_' ), array( ContentProcess_GetGetPartUri( $ctxProcess, '{id}.html' ) ), ";(function(f,g,r){function p(a,b,d,c){var h=b.getAttribute(\"data-lzl-nos\");if(h){var e=b.getAttribute(\"data-cp\");if(e)c&&k++,d=new g.XMLHttpRequest,d.open(\"GET\",\"_URL_GET_CONTPARTS_\".replace(\"%7Bid%7D\",e),!0),d.onload=function(){if(200==this.status){try{b.outerHTML=this.responseText}catch(q){}g.lzl_lazySizes.fire(a,\"seraph_accel_lzlNosLoaded\",{},!1,!0)}c&&(k--,!k&&l&&(l(),l=void 0))},d.send();else if(e=b.parentNode.querySelector('noscript[data-lzl-nos=\"'+h+'\"]'))b.outerHTML=e.textContent,e.parentNode.removeChild(e),\nd&&(d.fire(a,\"seraph_accel_lzlNosLoaded\",{},!1,!0),d.fire(g,\"resize\",{},!1,!0))}}function m(a,b,d){if(void 0!==b){if(\"string\"!==typeof b)return;var c=b.indexOf(\"#\");if(-1==c)return;c=b.substr(c+1)}if(void 0===c||!a.querySelector('[id=\"'+c+'\"]')){b=g.lzl_lazySizes;for(var h=a.querySelectorAll(\"i\"+(d?\".bjs\":\".lzl\")+\"[data-lzl-nos]\"),e=0;e<h.length;e++){var q=h[e];q.classList.remove(\"lzl\");p(a,q,b,d);if(void 0!==c&&a.querySelector('[id=\"'+c+'\"]'))break}}}function t(a){f.removeEventListener(r,t,{capture:!0,\npassive:!0});u=!0;setTimeout(function(){var b=g.lzl_lazySizes,d;for(d in n){var c=n[d];p(c.ownerDocument,c,b,c.classList.contains(\"bjs\"))}n=void 0},0)}var k=0,l,u,n=[];g.lzl_lazysizesConfig.beforeUnveil=function(a,b){u?p(a.ownerDocument,a,b,a.classList.contains(\"bjs\")):a.getAttribute(\"data-lzl-nos\")&&n.push(a)};m(f,location.href);f.addEventListener(r,t,{capture:!0,passive:!0});f.addEventListener(\"click\",function(a){m(f,a.target.getAttribute(\"href\"))},{capture:!0,passive:!0});f.addEventListener(\"keydown\",\nfunction(a){(70==a.keyCode&&(a.ctrlKey||a.metaKey)||191==a.keyCode)&&m(f)},{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(a){m(f,void 0,!0);if(k)return l=a,!0},5)})(document,window,\"DOMContentLoaded\");" );
				}
		...
		if( (isset($settContPr[ 'earlyPaint' ])?$settContPr[ 'earlyPaint' ]:null) && !(isset($ctxProcess[ 'compatView' ])?$ctxProcess[ 'compatView' ]:null) && !$ctxProcess[ 'isAMP' ] )
		{

			{
				...
				$item -> setAttribute( 'src',
					 LazyLoad_SrcSubst( $ctxProcess, array( 'cx' => 10, 'cy' => 10 ) )
	...
	if( $ctxProcess[ 'mode' ] == 'fragments' )
	{
		...
		foreach( HtmlNd::ChildrenAsArr( $doc -> getElementsByTagName( 'script' ) ) as $item )
		  	if( !ContentProcess_IsItemInFragments( $ctxProcess, $item, true ) )
				$item -> parentNode -> removeChild( $item );
